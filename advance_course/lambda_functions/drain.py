...
def checkContainerInstanceTaskStatus(clusterListResp, Ec2InstanceId):
    ...
    for token in tmpList:
        if token.find(b'ECS_CLUSTER') > -1:
            # Split and get the cluster name
            clusterName = token.split(b'=')[1].decode('utf-8')
    ...
    for containerListResp in clusterListPages:
        containerDetResp = ecsClient.describe_container_instances(cluster=clusterName,
                                                                  containerInstances=
                                                                  clusterListResp[
        ...
        for containerInstances in containerDetResp['containerInstances']:
            logger.debug("Container Instance ARN: %s and ec2 Instance ID %s",
            ...
            if containerInstances['ec2InstanceId'] == Ec2InstanceId:
                ...
                if containerStatus == 'DRAINING':
                    logger.info("Container ID %s with EC2 instance-id %s is draining tasks",
                                containerInstanceId,
                                Ec2InstanceId)
    ...
    if containerInstanceId != None:
        ...
        listTaskResp = ecsClient.list_tasks(cluster=clusterName,
...
def lambda_handler(event, context):
    ...
    if 'LifecycleTransition' in message.keys():
        ...
        if message['LifecycleTransition'].find('autoscaling:EC2_INSTANCE_TERMINATING') > -1:
            ...
            tasksRunning, tmpMsgAppend = checkContainerInstanceTaskStatus(clusterListResp,
                                                                          Ec2InstanceId)
            ...
            if tasksRunning == 1:
                ...
                for key in response['Subscriptions']:
                    logger.info("Endpoint %s AND TopicArn %s and protocol %s ", key['Endpoint'],
                                key['TopicArn'],
                                key['Protocol'])
                    if TopicArn == key['TopicArn'] and key['Protocol'] == 'lambda':
                        ...
                        logger.debug("msgResponse %s and time is %s", msgResponse,
                                     datetime.datetime)
            # If tasks are NOT running...
            elif tasksRunning == 0:
                completeHook = 1
                logger.debug(
                    "Setting lifecycle to complete;No tasks are running on instance, completing lifecycle action....")
